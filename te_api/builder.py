...
def generate_from_spec(spec, output_dir, read_only=False, log=None):
    ...
    if read_only:
        ...
        if not unique_cmds:
            log("Warning: No GET operations found in the spec.")
            return
    ...
    log(f"Generating {registry_path}...")
...
_CREDENTIALS_HELP = (
    "TE-API needs to download the API schema before it can run, but no\n"
    "OAuth2 credentials are configured.\n"
    "\n"
    "Set them up via either:\n"
    "  - Environment variables:\n"
    "      export TRANSPARENT_CLIENT_ID=...\n"
    "      export TRANSPARENT_CLIENT_SECRET=...\n"
    "  - A .env file (in the current directory or in $HOME) with:\n"
    "      TRANSPARENT_CLIENT_ID=...\n"
    "      TRANSPARENT_CLIENT_SECRET=...\n"
    "\n"
    "Then run te-api again. The schema will be fetched automatically."
)
